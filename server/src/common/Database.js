...
const Database = {
  ...
  getDatabaseUrl: () => 'mongodb://localhost:27017/expense_tracker',
  ...
  initialize: () => Database.getMongoClientPromise()
    .then(({db}) => Database.createUniqueIndexPromise({db, collection: 'users', index: 'username'}))
    .then(({db}) => Database.createUniqueIndexPromise({db, collection: 'sessions', index: 'token'}))
    .then(({db}) => Database.createUniqueIndexPromise({db, collection: 'records', index: 'id'}))
    .then(({db}) => db.close())
    .catch(Database.printErrorAndQuit),
