...
test.before(t => {
  Target.getDatabaseUrl = () => 'mongodb://localhost:27017/expense_tracker_DatabaseTest';
});
...
test.serial('DatabaseTest: getDatabaseWithIndex', async t => {
  ...
  const collections = (await db.collections())
    .filter(({namespace}) => namespace !== 'expense_tracker_DatabaseTest.system.indexes');
  t.is(collections.length, 1, 'only one collection');
  t.is(collections[0].namespace, 'expense_tracker_DatabaseTest.collection_name', 'collection name is correct');
...
test.serial('getAllConnectionsPromise for database with 1 collection', async t => {
  ...
  t.deepEqual(Objects.filterKeys(object, ['db', 'collections']), object, 'object only has these fields');
  t.deepEqual(object.db, db);
  const collections = object.collections.filter(({namespace}) => namespace !== 'expense_tracker_DatabaseTest.system.indexes');
  t.is(collections.length, 1);
  t.is(collections[0].namespace, 'expense_tracker_DatabaseTest.collection_name');
  await db.close();
});
...
test.serial('collectionDropPromise', async t => {
  ...
  t.is(collectionsBefore[0].namespace, 'expense_tracker_DatabaseTest.collection_name')
...
test.serial('resetCollectionPromise', async t => {
  const db = await getDatabaseAfterTwoInsertions();
  const documentsBefore = await db.collection('collection_name').find({}).toArray();
  t.is(documentsBefore.length, 2, 'exactly two document');
  t.is(documentsBefore[0].index_name, 'unique_value_name');
  ...
  await Target.resetCollectionPromise('collection_name');
...
test.serial('initialize', async t => {
  ...
  const collections = (await db.collections())
    .filter(({namespace}) => namespace !== 'expense_tracker_DatabaseTest.system.indexes');
  t.is(collections.length, 3, 'exactly two collections');
  const namespaces = collections.map(collection => collection.namespace);
  t.true(namespaces.indexOf('expense_tracker_DatabaseTest.records') !== -1);
  t.true(namespaces.indexOf('expense_tracker_DatabaseTest.users') !== -1);
  t.true(namespaces.indexOf('expense_tracker_DatabaseTest.sessions') !== -1);
  t.deepEqual(await db.collection('records').indexInformation(), { _id_: [ [ '_id', 1 ] ], id_1: [ [ 'id', 1 ] ] });
  t.deepEqual(await db.collection('sessions').indexInformation(), { _id_: [ [ '_id', 1 ] ], token_1: [ [ 'token', 1 ] ] });
  t.deepEqual(await db.collection('users').indexInformation(), { _id_: [ [ '_id', 1 ] ], username_1: [ [ 'username', 1 ] ] });
  t.deepEqual(await db.collection('records').find({}).toArray(), [], 'no documents');
  t.deepEqual(await db.collection('sessions').find({}).toArray(), [], 'no documents');
